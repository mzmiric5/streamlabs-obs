...
export class PlatformAppsService extends StatefulService<IPlatformAppServiceState> {
  ...
  async initialize() {
    ...
    if (this.state.devMode && localStorage.getItem(this.unpackedLocalStorageKey)) {
      ...
      if (data.appPath && data.appToken) {
        this.loadUnpackedApp(data.appPath, data.appToken, data.appCustomUrl);
      }
    }
  }
   ...
   */
  async loadUnpackedApp(appPath: string, appToken: string, appCustomUrl: string = null) {
  ...
  async refreshApp(appId: string) {
    ...
    if (app.unpacked) {
      const error = await this.loadUnpackedApp(app.appPath, app.appToken, app.appCustomUrl);
