...
export class PlatformContainerManager {
  ...
  getAssetUrl(app: ILoadedApp, asset: string) {
    ...
    if (app.unpacked) {
      url = this.getAssetUrlWithRemoteSupport(app, asset);
    } else {
  ...
  getAssetUrlWithRemoteSupport(app: ILoadedApp, asset: string) {
    const trimmed = app.appCustomUrl ? '' : trim(app.manifest.buildPath, '/ ');
    return compact([app.appCustomUrl ? app.appCustomUrl : `http://localhost:${app.devPort}`, trimmed, asset]).join('/');
  }
