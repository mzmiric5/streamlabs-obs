    ...
    <div slot="content">
      ...
      <div v-if="!infoLoading && !infoError && !populatingModels">
        ...
        <h-form-group
          ...
          :metadata="{ type: 'text', name: 'stream_title', title: $t('Title') }"
        />
        <h-form-group
        ...
        <div v-if="areAvailableProfiles">
            ...
            <div class="input-wrapper">
              <div class="checkbox">
                <div v-if="!isGenericProfiles">
                  <input type="checkbox" v-model="useOptimizedProfile" />
                  <label
                    ><span>Use {{ gameModel.value }} encoder settings</span></label
                  >
                </div>
                <div v-else>
                  <input type="checkbox" v-model="useOptimizedProfile" />
                  <label
                    ><span>{{ $t('Use optimized encoder settings') }}</span>
                    <span>
                      <i
                        class="tooltip-trigger fa fa-question-circle has-tooltip"
                        style="font-size:15px;whitespace: nowrap;"
                        :title="
                          $t(
                            'Optimized encoder gives equivalent quality while reducing usage. Game specific encoders for Fortnite, PUBG, Destiny 2, and League Of Legends',
                          )
                        "
                      >
                      </i>
          ...
          <div class="input-container select" v-show="useOptimizedProfile">
            <div class="input-label"><label>Profile</label></div>
            <div class="input-wrapper">
              <multiselect
                :allowEmpty="false"
                :options="profiles"
              ...
              >
                <template slot="option" slot-scope="props">
                  ...
                  <div class="edit-stream-info-option-longdesc">
